...
export function buildNarrationPlan({scenes,fonts,style}) {
 ...
 const filters=scenes.map((s,i)=>{
   ...
   const captions=(s.captionFiles||[]).map(c=>`drawtext=fontfile='${escapePath(fonts.bold)}':textfile='${escapePath(c.path)}':expansion=none:fontsize=${Math.min(124,Math.floor(1000/Math.max(1,...String(c.text||'').split('\n').map(line=>[...line].length))))}:fontcolor=${s.role==='closing'?(style.closingColor||'0xFF90CC'):(style.fontColor||'white')}:borderw=${style.borderWidth??5}:bordercolor=${style.borderColor||'0x28202A'}${style.boxColor?`:box=1:boxcolor=${s.role==='closing'?'0x58352D':style.boxColor}:boxborderw=22`:''}:shadowcolor=black@0.35:shadowx=2:shadowy=3:line_spacing=14:x=(w-text_w)/2:y=(h-text_h)/2:enable='gte(t,${c.start})*lt(t,${c.end})'`);
   const transition=style.purpose==='brand' ? (i>0?',fade=t=in:st=0:d=0.15':'')+(i<scenes.length-1?`,fade=t=out:st=${s.duration-0.15}:d=0.15`:'') : '';
   return base+movement+transition+(captions.length?','+captions.join(','):'')+`,format=yuv420p,setpts=PTS-STARTPTS[v${i}]`;
 });
